...
class SphinxClient
{
    ...
    public function _GetResponse($fp, $client_ver)
    {
        ...
        // check status
        if (SEARCHD_WARNING == $status) {
            list(, $wlen) = unpack('N*', substr($response, 0, 4));
            $this->_warning = substr($response, 4, $wlen);

            return substr($response, 4 + $wlen);
        }
        if (SEARCHD_ERROR == $status) {
            $this->_error = 'searchd error: '.substr($response, 4);

            return false;
        }
        if (SEARCHD_RETRY == $status) {
            $this->_error = 'temporary searchd error: '.substr($response, 4);

            return false;
        }
        if (SEARCHD_OK != $status) {
    ...
    {
        assert(SPH_MATCH_ALL == $mode
            || SPH_MATCH_ANY == $mode
            || SPH_MATCH_PHRASE == $mode
            || SPH_MATCH_BOOLEAN == $mode
            || SPH_MATCH_EXTENDED == $mode
            || SPH_MATCH_FULLSCAN == $mode
            || SPH_MATCH_EXTENDED2 == $mode);
        $this->_mode = $mode;
    }
    ...
    {
        assert(SPH_RANK_PROXIMITY_BM25 == $ranker
            || SPH_RANK_BM25 == $ranker
            || SPH_RANK_NONE == $ranker
            || SPH_RANK_WORDCOUNT == $ranker
            || SPH_RANK_PROXIMITY == $ranker);
        $this->_ranker = $ranker;
    }
    ...
    {
        assert(
            SPH_SORT_RELEVANCE == $mode ||
            SPH_SORT_ATTR_DESC == $mode ||
    ...
    public function SetGroupBy($attribute, $func, $groupsort = '@group desc')
    {
        ...
        assert(SPH_GROUPBY_DAY == $func
            || SPH_GROUPBY_WEEK == $func
            || SPH_GROUPBY_MONTH == $func
            || SPH_GROUPBY_YEAR == $func
            || SPH_GROUPBY_ATTR == $func
            || SPH_GROUPBY_ATTRPAIR == $func);
    ...
    public function Query($query, $index = '*', $comment = '')
    {
        ...
        if (SEARCHD_ERROR == $results[0]['status']) {
            return false;
        }
