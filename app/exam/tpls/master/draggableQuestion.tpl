    ...
    createApp({
        ...
        methods: {
            saveAnswer() {
                ...
                $.post(dmethods.GetUrlRelativePath() + '?exam-master-questions-draggableQuestion&questionid=' + questionId, { data: answer, type: 'modify' }, (data, status) => {
                    if (data === 'ok') {
                        Swal.fire({
                            title: '成功!',
                            icon: 'success'
                        ...
                        Swal.fire({
                            title: '[Dx0011] 系统错误! 请联系站点管理员排除问题!',
                            icon: 'error'
                        })
                    }
                })
