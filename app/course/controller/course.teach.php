...
class action extends app
{
    ...
    private function edit()
    {
        ...
        if ($this->ev->get('submit')) {
            ...
            $parentcat = $this->category->getCategoriesByArgs([['AND', 'catparent = 0']]);
            $teachers = $this->user->getUsersByArgs([['AND', 'groupmoduleid = 12'], ['AND', 'usergroupid = groupid']]);
    ...
    private function index()
    {
        ...
        $args[] = ['AND', 'find_in_set(cssubjectid,:cssubjectid)', 'cssubjectid', $this->teachsubjects];
        if ($search['courseid']) {
