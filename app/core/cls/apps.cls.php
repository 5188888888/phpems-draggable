...
class apps_core
{
    ...
    {
        $data = [false, 'app', [['AND', 'appid = :appid', 'appid', $appid]]];
        $sql = $this->pdosql->makeSelect($data);

        return $this->db->fetch($sql, ['appsetting', 'appusersetting']);
    }

    //修改应用信息
    public function modifyApp($appid, $args)
    {
        $data = ['app', $args, [['AND', 'appid = :appid', 'appid', $appid]]];
    ...
    {
        $data = [false, 'app', $args, false, false, false];
        $sql = $this->pdosql->makeSelect($data);
        $r = $this->db->fetchAll($sql, 'appid', 'appsetting');
        foreach ($r as $key => $p) {
            if (!file_exists('app/'.$key)) {
                unset($r[$key]);
            }
        }
